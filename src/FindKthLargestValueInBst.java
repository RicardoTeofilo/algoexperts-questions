import java.util.ArrayList;

/**
 * <div class="html">
 * <h2>Find Kth Largest Value in BST</h2>
 * <p>
 *   Write a function that takes in a Binary Search Tree (BST) and a positive
 *   integer <span>k</span> and returns the kth largest integer contained in the
 *   BST.
 * </p>
 * <p>
 *   You can assume that there will only be integer values in the BST and that
 *   <span>k</span> is less than or equal to the number of nodes in the tree.
 * </p>
 * <p>
 *   Also, for the purpose of this question, duplicate integers will be treated as
 *   distinct values. In other words, the second largest value in a BST containing
 *   values <span>{5, 7, 7}</span> will be <span>7</span>—not <span>5</span>.
 * </p>
 * <p>
 *   Each <span>BST</span> node has an integer <span>value</span>, a
 *   <span>left</span> child node, and a <span>right</span> child node. A node is
 *   said to be a valid <span>BST</span> node if and only if it satisfies the BST
 *   property: its <span>value</span> is strictly greater than the values of every
 *   node to its left; its <span>value</span> is less than or equal to the values
 *   of every node to its right; and its children nodes are either valid
 *   <span>BST</span> nodes themselves or <span>None</span> / <span>null</span>.
 * </p>
 * <h3>Sample Input</h3>
 * <pre><span class="CodeEditor-promptParameter">tree</span> =
 *           15
 *        /     \
 *       5      20
 *     /   \   /   \
 *    2     5 17   22
 *  /   \
 * 1     3
 * <span class="CodeEditor-promptParameter">k</span> = 3
 * </pre>
 * <h3>Sample Output</h3>
 * <pre>17</pre>
 * </div>
 */
public class FindKthLargestValueInBst {

    // This is an input class. Do not edit.
    static class BST {
        public int value;
        public BST left = null;
        public BST right = null;

        public BST(int value) {
            this.value = value;
        }
    }

    public int findKthLargestValueInBst(BST tree, int k) {
        ArrayList<Integer> inOderElementsList = new ArrayList<>();
        inOrderBSTTraversal(tree, inOderElementsList);
        return inOderElementsList.get(inOderElementsList.size() - k);
    }

    public void inOrderBSTTraversal(BST tree, ArrayList<Integer> inOderElementsList){

        if(tree == null)
            return;

        inOrderBSTTraversal(tree.left, inOderElementsList);
        inOderElementsList.add(tree.value);
        inOrderBSTTraversal(tree.right, inOderElementsList);
    }

    public int findKthLargestValueInBstReverseOrder(BST tree, int k) {

        TreeInfo treeInfo = new TreeInfo(-1, 0);
        reverseOrderBSTTraversal(tree, treeInfo, k);
        return treeInfo.lastVisitedNode;
    }

    public void reverseOrderBSTTraversal(BST tree, TreeInfo treeInfo, int k){

        if(tree == null || treeInfo.numberOfNodesVisited >= k)
            return;

        reverseOrderBSTTraversal(tree.right, treeInfo, k);
        if(treeInfo.numberOfNodesVisited < k){
            treeInfo.numberOfNodesVisited++;
            treeInfo.lastVisitedNode = tree.value;
            reverseOrderBSTTraversal(tree.left, treeInfo, k);
        }
    }

    static class TreeInfo{
        public int lastVisitedNode = -1;
        public int numberOfNodesVisited = 0;

        public TreeInfo(int lastVisitedNode, int numberOfNodesVisited) {
            this.lastVisitedNode = lastVisitedNode;
            this.numberOfNodesVisited = numberOfNodesVisited;
        }
    }


}
